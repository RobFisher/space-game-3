...
- Add a `where` command that reports a simple authoritative location summary for the current observer or a named object at the effective simulation time, with optional timestamp support for object queries.
...
- `authoritative-space-server`: Server distance handling changes from raw fixed-observer vector math to spatial-state-derived distance math, and command handling gains `where` plus `where <object> [--at <timestamp>]`.
